...
void process_transaction(transaction &trans)
{
  ...
  int n=0;
  {
    ...
    for ( auto cl_hist_it = day_transactions[trans.target].begin();cl_hist_it<day_transactions[trans.target].end();cl_hist_it++)
    {
    ...
    for ( auto cl_hist_it = filtered_history.begin();cl_hist_it<filtered_history.end();cl_hist_it++)
    {
      if ( fabs(cl_hist_it->amount - trans.amount) < fmax(cl_hist_it->amount,trans.amount) * eps +eps)
        same_amounts++;
    }
    if ( same_amounts>=4 )
    {
    ...
    day_transactions[trans.target]=filtered_history;
    day_transactions[trans.target].push_back(trans);
  }
