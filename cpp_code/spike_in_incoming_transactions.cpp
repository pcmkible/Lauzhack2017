...
void detect_incoming_spikes()
{
  ...
  for (auto it = sorted_transactions.begin();it< sorted_transactions.end();it++)
  {
    ...
    for ( auto cl_hist_it = day_transactions[it->target].begin();cl_hist_it<day_transactions[it->target].end();cl_hist_it++)
    {
    ...
    day_transactions[it->target]=filtered_history;
    day_transactions[it->target].push_back(*it);
  }
